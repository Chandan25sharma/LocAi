...
public class MessageDao_Impl(
  ...
  init {
    ...
    this.__insertAdapterOfMessageEntity = object : EntityInsertAdapter<MessageEntity>() {
      ...
      protected override fun bind(statement: SQLiteStatement, entity: MessageEntity) {
        ...
        val _tmp: String = __converters.fromMessageRole(entity.role)
  ...
  public override fun observeForConversation(conversationId: Long): Flow<List<MessageEntity>> {
  ...
  public override suspend fun getForConversation(conversationId: Long): List<MessageEntity> {
    val _sql: String = "SELECT * FROM messages WHERE conversationId = ? ORDER BY timestamp ASC"
  ...
  public override suspend fun getRecentForCategory(
    categoryId: String,
    excludeConversationId: Long,
    limit: Int,
  ): List<MessageEntity> {
    val _sql: String =
        "SELECT * FROM messages WHERE categoryId = ? AND conversationId != ? ORDER BY timestamp DESC LIMIT ?"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
        ...
        _argIndex = 2
        _stmt.bindLong(_argIndex, excludeConversationId)
        _argIndex = 3
  ...
  public override suspend fun deleteForConversation(conversationId: Long) {
